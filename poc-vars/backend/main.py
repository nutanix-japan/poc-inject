from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
import json
from pathlib import Path

app = FastAPI(title="Labs API")

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

LABS_FILE = Path("labs.json")


def load_lab():
    with open(LABS_FILE, "r", encoding="utf-8") as f:
        return json.load(f)


def save_lab(data):
    with open(LABS_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2)


# ── Get full lab data (columns + all slots) ──
@app.get("/api/lab")
def get_lab():
    return load_lab()


# ── Get variables for a specific email (used by frontend for [[key]] replacement) ──
@app.get("/api/variables")
def get_variables(email: str = None):
    data = load_lab()
    
    if not data.get("slots"):
        return {"variables": {}}
    
    # Find slot by email, fallback to first slot
    slot = None
    if email:
        slot = next((s for s in data["slots"] if s.get("email") == email), None)
    
    if not slot:
        slot = data["slots"][0]   # default fallback

    # Return flat object including the email field
    variables = {k: v for k, v in slot.items()}
    return {"variables": variables}


# ── Validate email exists ── (Fixed & Cleaned)
@app.get("/api/validate-email")
def validate_email(email: str):
    if not email:
        return {"valid": False, "message": "Email parameter is required"}
    
    data = load_lab()
    exists = any(slot.get("email") == email for slot in data.get("slots", []))
    
    if exists:
        return {"valid": True}
    else:
        return {"valid": False, "message": "Email not allocated for labs. Contact Instructor."}


# ── CRUD Operations (kept but cleaned) ──

@app.post("/api/lab/slots/{email}")
def upsert_slot(email: str, updated: dict):
    data = load_lab()
    for i, slot in enumerate(data["slots"]):
        if slot.get("email") == email:
            data["slots"][i] = {**updated, "email": email}  # ensure email is preserved
            save_lab(data)
            return {"status": "updated"}
    
    # Insert new slot
    new_slot = {**updated, "email": email}
    data["slots"].append(new_slot)
    save_lab(data)
    return {"status": "inserted"}


@app.delete("/api/lab/slots/{email}")
def delete_slot(email: str):
    data = load_lab()
    original_len = len(data["slots"])
    data["slots"] = [s for s in data["slots"] if s.get("email") != email]
    
    if len(data["slots"]) == original_len:
        return {"error": "Slot not found"}
    
    save_lab(data)
    return {"status": "deleted"}


@app.post("/api/lab/columns")
def add_column(payload: dict):
    col_name = payload.get("column")
    if not col_name:
        return {"error": "Missing column name"}
    
    data = load_lab()
    if col_name in data["columns"]:
        return {"error": "Column already exists"}
    
    data["columns"].append(col_name)
    for slot in data["slots"]:
        slot.setdefault(col_name, "")
    
    save_lab(data)
    return {"status": "column added", "column": col_name}


@app.delete("/api/lab/columns/{col_name}")
def delete_column(col_name: str):
    data = load_lab()
    if col_name not in data["columns"]:
        return {"error": "Column not found"}
    
    data["columns"].remove(col_name)
    for slot in data["slots"]:
        slot.pop(col_name, None)
    
    save_lab(data)
    return {"status": "column deleted", "column": col_name}


if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=9000)